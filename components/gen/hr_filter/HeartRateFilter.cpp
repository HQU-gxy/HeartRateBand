...
// Return Type  : void
//
void HeartRateFilter::hr_filter(const float x_data[], const int x_size[2],
                                float y_data[], int y_size[2])
{
  static const signed char iv[6]{1, 0, -1, 1, 0, -1};
  ...
  if (!static_cast<int>(pd_.Hd_not_empty)) {
    ...
    for (i = 0; i < 6; i++) {
      pd_.Hd.cSFunObject.P1_RTP1COEFF[i] = static_cast<float>(iv[i]);
    }
