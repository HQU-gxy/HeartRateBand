...
void HeartRateFilter::hr_filter(const real32_T x_data[], const int32_T x_size,
                                real32_T y_data[])
{
  // NOTE: I modify the generated code and move the initialization code to a
  // lambda function to dynamically allocate memory.
  static auto hr_filter_init = [](HeartRateFilter *aInstancePtr,
                                  size_t x_size) -> size_t {
    hr_filterStackData &localSD = *aInstancePtr->getStackData();
    auto &state = localSD.pd->Hd.cSFunObject.W0_FILT_STATES;
    const auto expected_size = x_size * 4;
    if (*state.size() != expected_size) {
      state.set_size(expected_size);
    }
    localSD.pd->Hd_not_empty = false;
    return expected_size;
  };

  const auto expected_size = hr_filter_init(this, x_size);
  ...
  if (pd_.Hd.cSFunObject.W1_PreviousNumChannels == -1) {
    pd_.Hd.cSFunObject.W1_PreviousNumChannels = x_size;
  }
